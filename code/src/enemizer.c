...
#define BgCheck_EntityLineTest1 ((s32(*)(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult, \
...
s16 Skullwalltula_GetTargetRotation(s16 orig, EnSw* walltula) {
    static u8 i = 0;
    CitraPrint("Skullwalltula_GetTargetRotation %X %X", i++, walltula->action_fn);
    if (gSettingsContext.enemizer == OFF) {
        return orig;
    }
    if (walltula->action_fn == (void*)0x3C2A50) {
        i = 0;
