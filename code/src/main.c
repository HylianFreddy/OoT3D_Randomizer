...
void Debug_Init(void) {
    // The memory left unused by the game after the .bss section continues until address 0x07FFFFFF.
    extern volatile const u32 numCustomMessageEntries;
    extern volatile const MessageEntry* ptrCustomMessageEntries;
    if (numCustomMessageEntries > 0) {
        CitraPrint("End of custom message data at address 0x%X",
                   ptrCustomMessageEntries + sizeof(MessageEntry) * numCustomMessageEntries);
    }
